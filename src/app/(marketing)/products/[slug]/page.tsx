...
import { ArrowLeft, ArrowRight, Check, Package, Zap } from "lucide-react"
...
export default async function ProductPage({ params }: Props) {
    ...
    const pricing = (product.pricing ?? []) as { label: string; price: string; features: string[]; highlight?: boolean; note?: string }[]
    const techHighlights = (product.techHighlights ?? []) as string[]
                ...
                {related.length > 0 && (
                    <section>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                            {related.map((p) => {
                                const RelIcon = PRODUCT_ICONS[p.icon] ?? Package
                                return (
                                    <Link
                                        key={p.slug}
