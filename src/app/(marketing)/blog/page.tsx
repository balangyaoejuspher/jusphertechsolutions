...
import { allPosts, featuredPost } from "@/lib/posts"

const categories = [
  { label: "All", count: 7 },
  { label: "Outsourcing", count: 3 },
  { label: "Blockchain & Web3", count: 2 },
...
export default function BlogPage() {
  const excludedSlugs = ["why-philippine-developers-are-world-class"]
  return (
      ...
      <div className="container mx-auto px-6 md:px-12 py-12">
        ...
        <div className="flex flex-wrap gap-2 mb-12">
          {categories.map((cat) => (
            <button
              key={cat.label}
              className={`px-4 py-2 rounded-xl text-sm font-medium border transition-all ${cat.label === "All"
                ...
                <div className="flex items-center justify-between flex-wrap gap-4">
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-full bg-amber-400/20 border border-amber-400/30 flex items-center justify-center">
                      <span className="text-amber-400 text-xs font-black">J</span>
                    </div>
                    <div>
                      <p className="text-white text-sm font-semibold leading-none">{featuredPost.author}</p>
                      <p className="text-zinc-500 text-xs mt-0.5">{featuredPost.role}</p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-between mt-auto pt-4 border-t border-zinc-100 dark:border-white/5">
                    <div className="flex items-center gap-2">
                      <div className="w-6 h-6 rounded-full bg-zinc-200 dark:bg-zinc-700 flex items-center justify-center">
                        <span className="text-zinc-600 dark:text-zinc-400 text-[10px] font-black">J</span>
                      </div>
