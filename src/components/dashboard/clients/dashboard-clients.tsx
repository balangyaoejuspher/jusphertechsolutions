...
import type { ClientFormState, ClientStatus, ClientType } from "@/types"
import {
    Building2, Check, Globe, Mail, MapPin,
    MoreHorizontal, Pencil, Phone, Plus,
        ...
        <div className="flex flex-col gap-6 p-6 md:p-8">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
                {Array.from({ length: 5 }).map((_, i) => (
                    <div key={i} className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-white/5 rounded-2xl p-4 text-center">
...
function ClientModal({
    ...
    const [form, setForm] = useState<ClientFormState>(
        client
            ? { ...client, password: "" }
            : { type: "company", status: "prospect", services: [], password: "" }
    )
    const [saving, setSaving] = useState(false)

    const set = (key: keyof ClientFormState, value: unknown) =>
        setForm((prev) => ({ ...prev, [key]: value }))

    const toggleService = (s: string) => {
        const current = form.services ?? []
        set("services", current.includes(s)
    ...
    const handleSave = async () => {
        ...
        try {
            if (!client) {
                ...
                const created = await portalFetch.post<ClientRow>("/admin/clients", {
                    clerkUserId: userId,
                ...
                onSave(created)
                toast.success(`Client ${form.name} has been created successfully!`)
            } else {
                const updated = await portalFetch.patch<ClientRow>(`/admin/clients/${client.id}`, {
                    type: form.type ?? client.type,
                    ...
                    company: form.company ?? null,
                    position: form.position ?? null,
                    status: form.status ?? client.status,
                    services: form.services ?? [],
                    notes: form.notes ?? null,
                })

                onSave(updated)
                toast.success(`Client ${form.name} has been updated successfully!`)
            }
                ...
                <div className="px-6 py-5 flex flex-col gap-4 max-h-[70vh] overflow-y-auto">
                    ...
                    {!client && (
                        <div>
                            ...
                            <input className={inputClass} type="password" placeholder="Temporary password for portal access" value={form.password ?? ""} onChange={(e) => set("password", e.target.value)} />
                        </div>
                    )}
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        ...
                        <div>
                            ...
                            <input className={inputClass} placeholder="+1 555 000 0000" value={form.phone ?? ""} onChange={(e) => set("phone", e.target.value || null)} />
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        <div>
                            ...
                            <input className={inputClass} placeholder="company.com" value={form.website ?? ""} onChange={(e) => set("website", e.target.value || null)} />
                        </div>
                        <div>
                            <label className={labelClass}>Location</label>
                            <input className={inputClass} placeholder="City, Country" value={form.location ?? ""} onChange={(e) => set("location", e.target.value || null)} />
                        </div>
                    </div>
                        ...
                        <div className="grid grid-cols-3 gap-2">
                            {(["active", "prospect", "inactive"] as ClientStatus[]).map((s) => (
                                <button key={s} onClick={() => set("status", s)} className={cn(
                                    ...
                                    <span className={cn("w-1.5 h-1.5 rounded-full shrink-0", form.status === s ? CLIENT_STATUS_CONFIG[s].dot : "bg-zinc-300 dark:bg-zinc-600")} />
                                    {CLIENT_STATUS_CONFIG[s].label}
                                </button>
                            ))}
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div className="flex flex-wrap gap-2">
                            {CLIENT_AVAILABLE_SERVICES.map((s) => {
    ...
    return (
        <div className="group bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-white/5 rounded-2xl p-5 hover:border-zinc-300 dark:hover:border-white/10 hover:shadow-sm transition-all duration-200">
            <div className="flex items-start justify-between gap-3 mb-4">
                ...
                <div className="flex items-center gap-2 shrink-0">
                    ...
                    <div className="relative">
                        <button onClick={() => setMenuOpen(!menuOpen)} className="w-7 h-7 flex items-center justify-center rounded-lg text-zinc-400 hover:bg-zinc-100 dark:hover:bg-white/5 transition-colors">
                        ...
                        {menuOpen && (
                            <>
                                ...
                                <div className="absolute right-0 top-8 z-20 w-36 bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-white/10 rounded-xl shadow-lg overflow-hidden">
                                    <button onClick={() => { onEdit(); setMenuOpen(false) }} className="flex items-center gap-2 w-full px-3 py-2.5 text-sm text-zinc-600 dark:text-zinc-300 hover:bg-zinc-50 dark:hover:bg-white/5 transition-colors">
                                        <Pencil size={13} /> Edit
                                    </button>
                                    <button onClick={() => { onDelete(); setMenuOpen(false) }} className="flex items-center gap-2 w-full px-3 py-2.5 text-sm text-red-500 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-400/5 transition-colors">
            ...
            <div className="flex items-center justify-between mt-3 pt-3 border-t border-zinc-100 dark:border-white/5">
                <span className="text-xs text-zinc-300 dark:text-zinc-700 capitalize">{client.type}</span>
                <span className="text-xs text-zinc-300 dark:text-zinc-700">
                    Since {formatDate(client.joinedDate)} {/* ← formatDate handles Date object */}
                </span>
            </div>
        </div>
    )
...
export default function DashboardClients() {
    ...
    }), [clients, search, filterStatus, filterType])
    ...
    const confirmDelete = async () => {
        if (!clientToDelete || confirmName !== clientToDelete.name) return
        setDeleting(true)
        try {
            await portalFetch.delete(`/admin/clients/${clientToDelete.id}`)
    ...
    if (loading) return <DashboardClientsSkeleton />
        ...
        <div className="flex flex-col gap-6 p-6 md:p-8">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    ...
                    <p className="text-sm text-zinc-500 dark:text-zinc-400 mt-1">Manage your individual and company clients</p>
                </div>
                <button
                        ...
                        <div className="flex flex-col gap-5">
                            <div>
                                ...
                                <p className="text-sm text-zinc-600 dark:text-zinc-400 mt-1">
                                    Type <strong className="text-zinc-900 dark:text-white">{clientToDelete.name}</strong> to confirm deletion.
                                </p>
                            </div>
                            <input
                                type="text"
                                ...
                                className={cn(
                                    "w-full px-4 py-3 rounded-xl border text-sm transition outline-none placeholder:text-zinc-400 dark:placeholder:text-zinc-600 bg-white dark:bg-zinc-800 text-zinc-900 dark:text-white",
                            ...
                            />
                            {confirmName && confirmName !== clientToDelete.name && (
                                <p className="text-xs text-red-500 -mt-3">Name does not match.</p>
                            )}
                            <div className="flex justify-end gap-3">
                                ...
                                <button
                                    ...
                                    disabled={confirmName !== clientToDelete.name || deleting}
                                    className="flex items-center gap-2 px-5 py-2 rounded-xl bg-red-500 hover:bg-red-400 disabled:opacity-40 disabled:cursor-not-allowed text-white text-sm font-bold transition"
                                >
